...
router
  ...
  .get(async (_, res) => {
    try {
      const articles = await prisma.chess_articles.findMany({
        orderBy: [
          {
            createdAt: "desc",
          },
  ...
  .post(checkAuthMiddleWare, async (req, res) => {
    ...
    const userMatch = await prisma.chess_users.findFirst({
      where: {
        username: req.token.username,
      },
    });
    if (userMatch && data.userID === userMatch?.id) {
      const newArticle = await prisma.chess_articles.create({
        data,
      });
...
router
  ...
  .get(async (req, res) => {
    ...
    const article = await prisma.chess_articles.findUnique({
      where: {
        id,
      },
    });
  ...
  .patch(checkAuthMiddleWare, async (req, res) => {
    ...
    if (userMatch && article && article?.userID === userMatch?.id) {
      const updatedArticle = await prisma.chess_articles.update({
        where: {
          id,
        },
        data,
  ...
  .delete(checkAuthMiddleWare, async (req, res) => {
    ...
    if (userMatch && article && article?.userID === userMatch?.id) {
      const deletedArticle = await prisma.chess_articles.delete({
        where: {
          id,
        },
      });
